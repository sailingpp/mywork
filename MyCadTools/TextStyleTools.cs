        ...
        public static ObjectId AddTextStyle(this Database db, string styleName, string fontFilename, string bigFontFilename)
        {
            ...
            using (Transaction trans = db.TransactionManager.StartTransaction())
            {
                ...
                if (!st.Has(styleName))//如果不存在名为styleName的文字样式，则新建一个文字样式
                {
                    ...
                    oid = st.Add(str);//将文字样式表记录的信息添加到文字样式表中
                    //把文字样式表记录添加到事务处理中
                ...
                {
                    TextStyleTable tsr = trans.GetObject(st[styleName], OpenMode.ForWrite) as TextStyleTable;
                    oid = st[styleName];
                }
                trans.Commit();
            }

            return oid;//返回新添加的文字样式表记录的ObjectId
        }
        public static ObjectId AddTextStyle1(this Database db, string styleName, string fontFilename, string bigFontFilename)
        {
            //打开文字样式表
            ObjectId oid = ObjectId.Null;
            using (Transaction trans = db.TransactionManager.StartTransaction())
            {
                ...
                {
                    TextStyleTableRecord str = trans.GetObject(st[styleName], OpenMode.ForWrite) as TextStyleTableRecord;
                    oid = st[styleName];
